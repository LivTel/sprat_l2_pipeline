...
int main (int argc, char *argv []) {
        ...
        if (argc != 11) {
                ...
                if(populate_env_variable(SPA_BLURB_FILE, "L2_SPA_BLURB_FILE")) {

                        RETURN_FLAG = 1;

                } else {

                        print_file(SPA_BLURB_FILE);

                }

                write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -1, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                ...
                if(!fits_open_file(&ext_arc_f_ptr, ext_arc_f, READONLY, &ext_arc_f_status)) {
                        ...
                        if(!populate_img_parameters(ext_arc_f, ext_arc_f_ptr, ext_arc_f_maxdim, &ext_arc_f_bitpix, &ext_arc_f_naxis, ext_arc_f_naxes, &ext_arc_f_status, "ARC FRAME")) {
                                ...
                                if (ext_arc_f_naxis != 2) {     // any data format checks here
                                        ...
                                        write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -2, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                                ...
                                write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -3, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                        ...
                        write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -4, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                        fits_report_error(stdout, ext_arc_f_status); 
                ...
                for (fpixel[1] = cut_y[0]; fpixel[1] <= cut_y[1]; fpixel[1]++) {
                        ...
                        if(!fits_read_pix(ext_arc_f_ptr, IMG_READ_ACCURACY, fpixel, nxelements, NULL, ext_arc_f_pixels, NULL, &ext_arc_f_status)) {
                                ...
                                write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -5, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                                fits_report_error(stdout, ext_arc_f_status); 
                ...
                if (!check_file_exists(arc_line_list_filename)) { 
                        ...
                        write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -7, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
		...
		double regression_alpha,regression_beta;				// Linear regression on the arc matching for diagnostics, not for calibration
                ...
                if (matched_line_count < min_matched_lines) {
                        ...
                        write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -8, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
		...
		regression_alpha = (sum_y/matched_line_count) - (regression_beta * sum_x/matched_line_count);
                ...
                // [SPARCFIT_OUTPUTF_WAVFITS_FILE] OUTPUT FILE
                // ***********************************************************************

                // 1.   Perform a few checks to ensure the input fitting parameters 
                //      are sensible

                if ((fit_order < SPARCFIT_VAR_POLYORDER_LO) || (fit_order > SPARCFIT_VAR_POLYORDER_HI)) {       // Check [fit_order] is within config limits

                        write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -9, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                ...
                // 2.   Create [SPARCFIT_OUTPUTF_WAVFITS_FILE] output file and print a few 
                //      parameters

                FILE *outputfile;
                outputfile = fopen(SPARCFIT_OUTPUTF_WAVFITS_FILE, FILE_WRITE_ACCESS);

                if (!outputfile) { 

                        write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -10, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                ...
                fprintf(outputfile, "#### %s ####\n\n", SPARCFIT_OUTPUTF_WAVFITS_FILE);
                fprintf(outputfile, "# List of pixel channel to wavelength dispersion solution coefficients and corresponding chi-squared found using the sparcfit program.\n\n");
                ...
                if (calc_least_sq_fit(fit_order, matched_line_count, coord_x, coord_y, coeffs, &this_chi_squared)) {    
                    ...
                    write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -11, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                ...
                // Output solutions to [SPARCFIT_OUTPUTF_WAVFITS_FILE] file 

                fprintf(outputfile, "%d\t", 1);

                for (ii=0; ii<=fit_order; ii++) {
                  
                    fprintf(outputfile, SPARCFIT_VAR_ACCURACY_COEFFS, coeffs[ii]);
                    fprintf(outputfile, "\t");

                }

                fprintf(outputfile, SPARCFIT_VAR_ACCURACY_CHISQ, this_chi_squared);
                fprintf(outputfile, "\n");
                ...
                if ((this_chi_squared < SPARCFIT_VAR_CHISQUARED_MIN) || (this_chi_squared > SPARCFIT_VAR_CHISQUARED_MAX)) {
                ...
                // [SPARCFIT_OUTPUTF_WAVFITS_FILE] file

                if (fclose(arc_line_list_f)) {

                        write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -12, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                ...
                if (fclose(outputfile)) {
                        ...
                        write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -13, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                ...
                if(fits_close_file(ext_arc_f_ptr, &ext_arc_f_status)) { 
                        ...
                        write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", -14, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
                        fits_report_error (stdout, ext_arc_f_status); 
                ...
                write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATAR", RETURN_FLAG, "Status flag for L2 sparcfit routine", ERROR_CODES_FILE_WRITE_ACCESS);
