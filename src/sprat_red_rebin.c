...
int main (int argc, char *argv []) {
	...
	if (argc != 8) {
		...
		write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -1, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
		...
		if(!fits_open_file(&input_f_ptr, input_f, READONLY, &input_f_status)) {
			...
			if(!populate_img_parameters(input_f, input_f_ptr, input_f_maxdim, &input_f_bitpix, &input_f_naxis, input_f_naxes, &input_f_status, "INPUT FRAME")) {
				...
				if (input_f_naxis != 2) {	// any data format checks here
					...
					write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -2, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
				...
				write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -3, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
			...
			write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -4, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
			fits_report_error(stdout, input_f_status); 
		...
		// ***********************************************************************
		// Open [SPARCFIT_OUTPUTF_WAVFITS_FILE] dispersion solutions file

		FILE *dispersion_solutions_f;
	
		if (!check_file_exists(SPARCFIT_OUTPUTF_WAVFITS_FILE)) { 

			dispersion_solutions_f = fopen(SPARCFIT_OUTPUTF_WAVFITS_FILE , "r");

		} else {

			write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -5, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
		...
		// ***********************************************************************
		// Find some [SPARCFIT_OUTPUTF_WAVFITS_FILE] file details
		...
		char search_string_1 [20] = "# Polynomial Order:\0";	// this is the comment to be found from the [SPARCFIT_OUTPUTF_WAVFITS_FILE] file
		...
		if (find_polynomialorder_comment == FALSE) {	// error check - didn't find the comment in the [SPARCFIT_OUTPUTF_WAVFITS_FILE] file

			write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -6, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
		...
		// ***********************************************************************
		// Rewind and extract coefficients from [SPARCFIT_OUTPUTF_WAVFITS_FILE]
		// file 
		...
		// ***********************************************************************
		// Find wavelength extremities from [SPARCFIT_OUTPUTF_WAVFITS_FILE] file
		...
		if (start_wav < smallest_wav) {         // Comparing doubles but accuracy isn't a necessity so don't need gsl_fcmp function
			...
			write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -7, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
			...
			write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -8, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
		...
		for (fpixel[1] = cut_y[0]; fpixel[1] <= cut_y[1]; fpixel[1]++) {
			...
			if(!fits_read_pix(input_f_ptr, IMG_READ_ACCURACY, fpixel, nxelements, NULL, input_f_pixels, NULL, &input_f_status)) {
				...
				if (interpolate(interpolation_type, x_wav, input_f_pixels, nxelements, bin_wavelengths[0], bin_wavelengths[num_bins-1], dispersion, output_f_pixels)) {
					...
					write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -9, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
				...
				write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -10, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
				fits_report_error(stdout, input_f_status);
		...
		// 6.	Create [SPREBIN_OUTPUTF_REBIN_WAVFITS_FILE] output file and print
		// 	a few parameters

		FILE *outputfile;
		outputfile = fopen(SPREBIN_OUTPUTF_REBIN_WAVFITS_FILE, FILE_WRITE_ACCESS);
		...
		fprintf(outputfile, "#### %s ####\n\n", SPREBIN_OUTPUTF_REBIN_WAVFITS_FILE);
		...
		write_additional_key_to_file_str(ADDITIONAL_KEYS_FILE, "LSS_CALIBRATION", "CUNIT1", "Angstrom", "Units for axis 1", ADDITIONAL_KEYS_FILE_WRITE_ACCESS);
		...
		write_additional_key_to_file_str(ADDITIONAL_KEYS_FILE, "SPEC_CALIBRATION", "CUNIT1", "Angstrom", "Units for axis 1", ADDITIONAL_KEYS_FILE_WRITE_ACCESS);
		...
		if (!fits_create_file(&output_f_ptr, output_f, &output_f_status)) {
			...
			if (!fits_create_img(output_f_ptr, INTERMEDIATE_IMG_ACCURACY[0], 2, output_f_naxes, &output_f_status)) {
				...
				if (!fits_write_img(output_f_ptr, INTERMEDIATE_IMG_ACCURACY[1], output_f_fpixel, num_bins * nyelements, output_frame_values_1D, &output_f_status)) {
					...
					write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -12, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
				...
				write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -13, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
				fits_report_error(stdout, output_f_status); 
			...
			write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -14, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
			fits_report_error(stdout, output_f_status); 
		...
		if (fclose(dispersion_solutions_f)) {
			...
			write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -15, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
		...
		if (fclose(outputfile)) {
			...
			write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -16, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
		...
		if(fits_close_file(input_f_ptr, &input_f_status)) { 
			...
			write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -17, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
			fits_report_error (stdout, input_f_status); 
		...
		if(fits_close_file(output_f_ptr, &output_f_status)) { 
			...
			write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", -18, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
			fits_report_error (stdout, output_f_status); 
		...
		write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRE", RETURN_FLAG, "Status flag for L2 sprebin routine", ERROR_CODES_FILE_WRITE_ACCESS);
