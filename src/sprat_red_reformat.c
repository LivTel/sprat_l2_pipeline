...
int main (int argc, char *argv []) {
	...
	if (argc != 5) {
		...
		if(populate_env_variable(SPRF_BLURB_FILE, "L2_SPRF_BLURB_FILE")) {

			RETURN_FLAG = 1;

		} else {

			print_file(SPRF_BLURB_FILE);

		}

		write_key_to_file(ERROR_CODES_FILE, REF_ERROR_CODES_FILE, "L2STATRF", -1, "Status flag for L2 spreformat routine", ERROR_CODES_FILE_WRITE_ACCESS);
