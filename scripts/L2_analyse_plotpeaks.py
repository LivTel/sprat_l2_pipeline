    ...
    with open(dat_peaks) as f:
        for line in f:
            if not line.startswith(
                    '#') and line.strip() != "" and not line.startswith('-1'):
...
if __name__ == "__main__":
    ...
    parser.add_option(
        '--p', dest='dat_peaks', action='store', default='spfind_peaks.dat')
    parser.add_option(
        '--t', dest='dat_traces', action='store', default='sptrace_traces.dat')
