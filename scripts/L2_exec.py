...
def rewrite_error_codes_file(error_codes_file,
                             old_header_key="",
                             new_header_key="",
                             add_to_desc="",
                             omit=False):
            ...
            for line in f_old:
                if omit:
                    ...
                    if not line.startswith(old_header_key):
                        ...
                        f_new.write(new_header_key + "\t" + code + "\t" +
                                    desc + " " + add_to_desc + "\n")
...
def search_lookup_table(lookup_table_path, this_DATEOBS, this_CCDXBIN,
    ...
    for i in range(len(a_path)):
        this_entry_from_time = time.strptime(
            a_from_date[i] + " " + a_from_time[i], "%d/%m/%y %H:%M:%S")
        this_entry_to_time = time.strptime(a_to_date[i] + " " + a_to_time[i],
                                           "%d/%m/%y %H:%M:%S")
...
def chk_ref_run(f_ref, f_cont):
    ...
    print_notification("Success. Using config.tab file " +
    ...
    rewrite_error_codes_file(
        cfg['general']['error_codes_file'],
        "L2STATTR",
        "L2STATT1",
        add_to_desc="(ref uncorrected)")
    ...
    output = Popen(
        [
            correct, in_ref_filename, cfg['spcorrect']['interpolation_type'],
            cfg['spcorrect']['conserve_flux'], out_ref_cor_filename
        ],
        stdout=PIPE)
    print output.stdout.read()
    output.wait()
    ...
    if output.returncode != 0:
        err.set_code(22, is_fatal=False)
    rewrite_error_codes_file(
        cfg['general']['error_codes_file'], "L2STATTR", omit=True)
    ...
    print_routine(
        "Plot trace of spectrum post sdist correction and check for curvature (l2pp)"
    )
    print
...
def full_run(f_target, f_ref, f_cont, f_arc, f_flcor, work_dir, clobber):
    ...
    SPEC_NONSS_plot = os.path.splitext(
        os.path.basename(output_target))[0] + "_SPEC_NONSS.png"
    SPEC_SS_plot = os.path.splitext(
        os.path.basename(output_target))[0] + "_SPEC_SS.png"
    ...
    spec_montage_plot2 = os.path.splitext(
        os.path.basename(output_target))[0] + "_spec_output_2.png"
    # Plots with the flux calibration
    FLUX_plot = os.path.splitext(
        os.path.basename(output_target))[0] + "_FLUX.png"
    NORMFLUX_plot = os.path.splitext(
        os.path.basename(output_target))[0] + "_NORMFLUX.png"
    ...
    try:
        if not os.path.exists(work_dir):
            os.mkdir(work_dir)
            copyfile(f_target,
                     work_dir + "/" + target + target_suffix + ".fits")
            copyfile(f_ref, work_dir + "/" + ref + ref_suffix + ".fits")
            copyfile(f_cont, work_dir + "/" + cont + cont_suffix + ".fits")
            ...
            if clobber:
                ...
                copyfile(f_arc, work_dir + "/" + arc + arc_suffix + ".fits")
                copyfile(f_flcor,
                         work_dir + "/" + flcor + flcor_suffix + ".fits")
            else:
                err.set_code(14)
    except OSError:
        err.set_code(15)
    ...
    with open("additional_keys", 'w') as f:
        f.write("str\tSTARTDATE\tL2DATE\t" + now_date + " " + now_time +
                "\twhen this reduction was performed\n")
    ...
    ## arc
    chosen_arc_entry = search_lookup_table(
        arc_tab_path, f_arc_fits_hdr_DATEOBS, f_arc_fits_hdr_CCDXBIN,
        f_arc_fits_hdr_CCDYBIN)
    ...
    ## config
    chosen_config_entry = search_lookup_table(
        config_tab_path, f_arc_fits_hdr_DATEOBS, f_arc_fits_hdr_CCDXBIN,
        f_arc_fits_hdr_CCDYBIN)
    if chosen_config_entry is None:
        print_notification("Failed.")
        err.set_code(29)
    ...
    output = Popen(
        [
            correct, in_target_filename,
            cfg['spcorrect']['interpolation_type'],
            cfg['spcorrect']['conserve_flux'], out_target_filename
        ],
        stdout=PIPE)
    print output.stdout.read()
    output.wait()
    ...
    output = Popen(
        [
            correct, in_ref_filename, cfg['spcorrect']['interpolation_type'],
            cfg['spcorrect']['conserve_flux'], out_ref_filename
        ],
        stdout=PIPE)
    print output.stdout.read()
    output.wait()
    ...
    ## continuum
    output = Popen(
        [correct, in_cont_filename, "linear", "1", out_cont_filename],
        stdout=PIPE)
    print output.stdout.read()
    output.wait()
    if output.returncode != 0:
    ...
    ## arc
    output = Popen(
        [correct, in_arc_filename, "linear", "1", out_arc_filename],
        stdout=PIPE)
    print output.stdout.read()
    output.wait()
    if output.returncode != 0:
    ...
    if output.returncode != 0:
        err.set_code(21, is_fatal=False)
    rewrite_error_codes_file(
        cfg['general']['error_codes_file'], "L2STATFI", omit=True)
    ...
    if output.returncode != 0:
        err.set_code(21, is_fatal=False)
    rewrite_error_codes_file(cfg['general']['error_codes_file'], "L2STATFI",
                             "L2STATF2", "(target corrected)")
    ...
    rewrite_error_codes_file(cfg['general']['error_codes_file'], "L2STATEX",
                             "L2STATX1", "(target NONSS)")
    ...
    try:
        ...
        f_target_fits_hdr_ACQFLUX = str(
            f_target_fits[0].header['ACQFAUTO']).strip()
        f_target_fits_hdr_ACQIMAGE = str(
            f_target_fits[0].header['ACQIMAGE']).strip()
    except:
    ...
    output = Popen(
        [
            spflcor, in_target_filename, f_target_fits_hdr_EXPTIME,
            cfg['sprebin']['start_wav'], cfg['sprebin']['dispersion'], "0",
            "0", "0", f_flcor, "1", out_target_filename
        ],
        stdout=PIPE)
    print output.stdout.read()
    output.wait()
    ...
    # f_target_fits_hdr_EXPTIME is actually no longer used in spflcor
        output = Popen(
            [
                spflcor, in_target_filename, f_target_fits_hdr_EXPTIME,
                cfg['sprebin']['start_wav'], cfg['sprebin']['dispersion'],
                f_target_fits_hdr_ACQFLUX, acq_min_lambda, acq_max_lambda,
                f_flcor, cfg['spflcor']['tel_thput'], out_target_filename
            ],
            stdout=PIPE)
        print output.stdout.read()
        output.wait()
    ...
    output = Popen(
        [
            reformat, in_target_filename_L1_IMAGE, in_target_headers_filename,
            "L1_IMAGE", out_target_filename
        ],
        stdout=PIPE)
    print output.stdout.read()
    ...
    for op in cfg['spreformat']['operations'].split(','):
        ...
        output = Popen(
            [
                reformat, in_f, in_target_headers_filename,
                op.strip(), out_target_filename
            ],
            stdout=PIPE)
        print output.stdout.read()
        output.wait()
    ...
    try:
        a_ps_execute(in_target_filename, "SPEC_NONSS", SPEC_NONSS_plot,
                     "SPEC_NONSS", "green", 1, "Intensity (counts)")
    except KeyError:
        pass
    ...
    try:
        ...
        a_ps_execute(in_target_filename, "FLUX", FLUX_plot, "FLUX", "magenta",
                     1, "Flux density ($\mathrm{erg\,s^{-1}cm^{-2}\AA^{-1}}$)")
    except KeyError:
        pass
    ...
    fig.suptitle(
        "Raster image of L1 IMAGE and extracted spectra\n " +
        in_target_filename + "   " + OBJECT,
        fontsize=12)
    fig.add_subplot(311)
    a_pi_execute(in_target_filename, "L1_IMAGE", "", "", save=False, hold=True)
    ...
    try:
        if not int(float(f_target_fits_hdr_ACQFLUX)) == 0:
            ...
            if OBJECT == "Hilt102":
                a_ps_execute(
                    L2_STANDARDS_DIR +
                    "/hiltner102_stone_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "GD50":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/gd50_oke90_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "BD_28_4211":
                a_ps_execute(
                    L2_STANDARDS_DIR +
                    "/bdp28d4211_oke90_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "BDp33_2642_zpol":
                a_ps_execute(
                    L2_STANDARDS_DIR +
                    "/bdp33d2642_oke90_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "G191B2B":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/g191b2b_oke90_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "LB1240":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/lb1240_oke74_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "HD19445" or OBJECT == "HD19445_m8":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/hd19445_oke83_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "GD108":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/gd108_oke90_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "Feige34" or OBJECT == "feige34":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/feige34_oke90_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "G60_54":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/g60_oke90_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "G193_74":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/g193_oke90_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "BDp75_325":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/bd75d325_oke90_ergscm2A_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            a_ps_execute(
                ...
                "blue",
                1,
                r'Flux density ($\mathrm{erg\,s^{-1}cm^{-2}\AA^{-1}}$)',
                telluric=True,
                legend=True,
                leg_title="FLUX",
                save=False,
            ...
            if OBJECT == "Hilt102":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/hiltner102_stone_flam_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "HD19445" or OBJECT == "HD19445_m8":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/hd19445_oke83_flam_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "BDp33_2642_zpol":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/bdp33d2642_oke90_flam_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "G191B2B":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/g191b2b_oke90_flam_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "G60_54":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/g60_oke90_flam_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
            ...
            if OBJECT == "G193_74":
                a_ps_execute(
                    L2_STANDARDS_DIR + "/g193_oke90_flam_9A_img.fits",
                    0,
                    "",
                    "",
                    "red",
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest='f_target',
        action='store',
        default=L2_TEST_DIR + "/1H0323/v_e_20141115_14_1_0_1.fits",
    ...
    parser.add_option(
        ...
        dest='f_cont',
        action='store',
        default=L2_TEST_DIR + "/1H0323/v_w_20141121_2_1_0_1.fits",
        help="path to continuum file")
    parser.add_option(
        '--a',
        dest='f_arc',
        action='store',
        default=L2_TEST_DIR + "/1H0323/v_a_20141115_15_1_0_1.fits",
        help="path to arc file")
    parser.add_option(
        '--f',
        dest='f_flcor',
        action='store',
        default=L2_TEST_DIR + "/1H0323/red_cal0_norm.fits",
        help="path to flux correction file")
    parser.add_option(
        '--dir',
        dest='work_dir',
        action='store',
